...
def fetch_tw_all(
    ...
    for code, quote in twse_quote.items():
        ...
        # 精確漲跌停：收盤價 vs 交易所公告的漲跌停價（容許 0.02 元誤差）
        lim   = twse_limits.get(code, {})
        close = quote.get("price", 0.0)
        lu    = lim.get("limit_up",   0.0)
        ld    = lim.get("limit_down", 0.0)
        is_lu = lu > 0 and abs(close - lu) <= 0.02
        is_ld = ld > 0 and abs(close - ld) <= 0.02
        result[yft] = {
            **quote,
            "fi_net":         fi["fi_net"],
            "it_net":         fi["it_net"],
    ...
    for code, quote in tpex_quote.items():
        ...
        # TPEX 無精確漲跌停端點，用漲幅近似（台股 10%，精度誤差最多 0.3%）
        dr    = quote.get("day_return", 0.0)
        result[yft] = {
            **quote, **chips, **margin,
            "inst_net":       inst,
            "it_consec_days": it_consec,
            "fi_consec_days": fi_consec,
            "is_limit_up":    dr >= 0.097,
            "is_limit_down":  dr <= -0.097,
            "ohlcv": ohlcv.get(yft),
        }
